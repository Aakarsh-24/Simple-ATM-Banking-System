		...
		if (ae.getSource()==b1)
		{
			...
			else if (ca[0]>='0' && ca[0]<='9')
			{
				...
				if (check2==cb.length && check1==ca.length && check1==16 && check2==4) {
					try {
					  FileWriter fr = new FileWriter("Database.txt", true);// Used to write on files
					  BufferedWriter br = new BufferedWriter(fr); 						 // Used to change input stream into writable format
					  br.write("1111"+" "+a+" "+b+" "+0+"\n");
